...
import math
from transformers.models.gpt2.modeling_gpt2 import GPT2LMHeadModel, GPT2Attention, GPT2Block, GPT2MLP, Conv1D, GPT2Model
import torch
...
class AverageSelfAttention(nn.Module):
    def __init__(self, attention_size, model_name):
...
class Cond_Attention(GPT2Attention):
    def __init__(self, nx, n_ctx, config, scale=False):
        ...
        self.head_dim = n_state // self.num_heads
        self.scale = scale
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None):
        w = torch.matmul(q, k.transpose(-1, -2))# [1 12 17 64], [1 12 64 18] # (batch, head, seq_length, head_features)
        if self.scale:
            w = w / math.sqrt(v.size(-1))# [1 12 17 18]
    ...
    def forward(self, x, z, layer_past=None, attention_mask=None, head_mask=None, use_cache=True):
        x = self.c_attn(x)
        ...
        if layer_past is not None:
            ...
            key = torch.cat((past_key, key), dim=-2)  #[1 12 17 64]
            value = torch.cat((past_value, value), dim=-2)
        present = torch.stack((key, value)) # .transpose(-2, -1) # transpose to have same shapes for stacking

        z_conv = self.c_z(z)
        key_z, value_z = z_conv.split(self.split_size, dim=2)
        key_z = self._split_heads(key_z, self.num_heads, self.head_dim)#[1 12 1 64]
        value_z = self._split_heads(value_z, self.num_heads, self.head_dim)#[1 12 1 64]
        key = torch.cat((key_z, key), dim=-2)#[1 12 18 64]
        value = torch.cat((value_z, value), dim=-2)#[1 12 18 64]
...
class Cond_Block(GPT2Block):
    ...
    def forward(self, x, z, layer_past=None, attention_mask=None, head_mask=None, use_cache=False):
        output_attn = self.attn(
            self.ln_1(x), z, layer_past=layer_past, attention_mask=attention_mask, head_mask=head_mask, use_cache=True
        )
...
class Encoder(GPT2Model):
    ...
    def forward(
        ...
        logvar = self.logvar(representations) # mean和logvar是一样的的conv1d
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if self.add_attn:
                if self.attn_proj_vary:
                    ...
                    z = attn_proj
                outputs = block(
                    hidden_states, z, layer_past=layer_past, attention_mask=attention_mask, head_mask=head_mask[i], use_cache=True
                )
            else:
                outputs = block(
                    hidden_states, layer_past=layer_past, attention_mask=attention_mask, head_mask=head_mask[i], use_cache=True
                )
...
class VAEModel(GPT2LMHeadModel):
    ...
    def forward(
        ...
        # train use posterior， test use prior
        ...
        if from_prior:# 始终是false, 因为generate(test) 的时候并不在这里进行，在外部(sample sequence)里直接调用encoder_prior
            latent_mean, latent_logvar = prior_mean, prior_logvar
        else: # train eval
            latent_mean, latent_logvar = posterior_mean, posterior_logvar
        #
        # if from_mean: # from_mean is set false
        #   z = latent_mean
        # else:
